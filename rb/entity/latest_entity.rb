...
class LatestEntity
  ...
  def make
    opts = @_entopts.dup
    LatestEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ExchangeRateHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ExchangeRateHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Latest.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Latest, Hash] the loaded Latest; raises ExchangeRateError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ExchangeRateHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

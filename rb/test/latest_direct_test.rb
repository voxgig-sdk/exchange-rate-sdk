...
class LatestDirectTest < Minitest::Test
  def test_direct_load_latest
    setup = latest_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-latest", setup[:live] ? "live" : "unit")
    if _should_skip
...
def latest_direct_setup(mockres)
  ...
  if live
    ...
    client = ExchangeRateSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,

...
The Python SDK for the ExchangeRate API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Latest(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.ExchangeRate(None).load(
    {"id": "test01"}, None
)

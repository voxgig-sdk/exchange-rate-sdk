...
class ExchangeRateTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ExchangeRateTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ExchangeRateTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ExchangeRateTestRunner.getenv("EXCHANGE_RATE_TEST_LIVE")
        override = ExchangeRateTestRunner.getenv("EXCHANGE_RATE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ExchangeRateTestRunner.getenv(key)
        ...
        explain = ExchangeRateTestRunner.getenv("EXCHANGE_RATE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["EXCHANGE_RATE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ExchangeRateTestRunner._test_control is not None:
            return ExchangeRateTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ExchangeRateTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ExchangeRateTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ExchangeRateTestRunner.load_test_control()
...
def load_env_local():
    ExchangeRateTestRunner.load_env_local()


def env_override(m):
    return ExchangeRateTestRunner.env_override(m)


def entity_data(v):
    return ExchangeRateTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ExchangeRateTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ExchangeRateTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ExchangeRateTestRunner.load_test_control()


def live_client_options():
    return ExchangeRateTestRunner.live_client_options()


def live_delay_ms():
    return ExchangeRateTestRunner.live_delay_ms()

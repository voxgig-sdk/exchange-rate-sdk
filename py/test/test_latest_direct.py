...
class TestLatestDirect:

    def test_should_direct_load_latest(self):
        setup = _latest_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-latest", "live" if setup["live"] else "unit")
        if _skip:
...
def _latest_direct_setup(mockres):
    ...
    if live:
        ...
        client = ExchangeRateSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,

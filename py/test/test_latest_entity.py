    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set EXCHANGE_RATE_TEST_LATEST_ENTID JSON to run live")
        client = setup["client"]
        ...
        latest_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(latest_ref01_data_dt0_loaded))
        assert latest_ref01_data_dt0_load_result is not None
        assert latest_ref01_data_dt0_load_result["id"] == latest_ref01_data["id"]
...
def _latest_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("EXCHANGE_RATE_TEST_LATEST_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("EXCHANGE_RATE_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("EXCHANGE_RATE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("EXCHANGE_RATE_TEST_EXPLAIN") == "TRUE",
        "live": _live,

...
import (
	...
	vs "github.com/voxgig-sdk/exchange-rate-sdk/go/utility/struct"
)

type LatestEntity struct {
	name    string
	client  *core.ExchangeRateSDK
...
func NewLatestEntity(client *core.ExchangeRateSDK, entopts map[string]any) *LatestEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *LatestEntity) GetName() string { return e.name }
...
func (e *LatestEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *LatestEntity) Deleted() bool {
	return e.deleted
}


func (e *LatestEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewLatestEntity(e.client, opts)
}

func (e *LatestEntity) Data(args ...any) any {
...
func (e *LatestEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Latest; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *LatestEntity) DataTyped(data ...Latest) Latest {
	if len(data) > 0 {
		return typedFrom[Latest](e.Data(asMap(data[0])))
	}
	return typedFrom[Latest](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Latest (all fields
// optional at the wire level).
func (e *LatestEntity) MatchTyped(match ...Latest) Latest {
	if len(match) > 0 {
		return typedFrom[Latest](e.Match(asMap(match[0])))
	}
	return typedFrom[Latest](e.Match())
}
...
func (e *LatestEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *LatestEntity) Load(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// LatestLoadMatch and returns an Latest. It delegates to the untyped
// Load (identical runtime) and converts at the typed boundary.
func (e *LatestEntity) LoadTyped(reqmatch LatestLoadMatch, ctrl map[string]any) (Latest, error) {
	res, err := e.Load(asMap(reqmatch), ctrl)
	if err != nil {
		return Latest{}, err
	}
	return typedFrom[Latest](res), nil
}



func (e *LatestEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}


func (e *LatestEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *LatestEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *LatestEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *LatestEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility

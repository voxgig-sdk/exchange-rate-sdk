...
local LatestEntity = {}
LatestEntity.__index = LatestEntity


function LatestEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LatestEntity)
  self._name = "latest"
  self._client = client
...
function LatestEntity:get_name()
  return self._name
end


function LatestEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LatestEntity.new(self._client, opts)
end
...
function LatestEntity:mark_deleted()
  self._deleted = true
end


function LatestEntity:deleted()
  return true == self._deleted
end


function LatestEntity:data_set(args)
  if args ~= nil then
...
function LatestEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LatestEntity:match_set(args)
  if args ~= nil then
...
function LatestEntity:match_get()
...
function LatestEntity:stream(action, args, callopts)
...
function LatestEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
[MCP](https://modelcontextprotocol.io) server exposing the ExchangeRate SDK as
two agent tools — `exchange-rate_list` and `exchange-rate_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/exchange-rate-mcp)
make build

# 2. Provide credentials via the environment
export EXCHANGE_RATE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user exchange-rate \
  -- /absolute/path/to/exchange-rate-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./exchange-rate-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// exchange-rate_list: first page of records
{ "entity": "latest" }
{ "entity": "latest", "query": { } }

// exchange-rate_load: one record by id
{ "entity": "latest", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/exchange-rate-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user exchange-rate \
     -- "$PWD"/dist/*/exchange-rate-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `exchange-rate_list` and `exchange-rate_load` tools now appear
   in new sessions. Ask the agent to *"list latest using exchange-rate"*
   and it calls `exchange-rate_list` with `{"entity":"latest"}`.
...
```sh
export EXCHANGE_RATE_APIKEY=sk_live_xxx            # API key
export EXCHANGE_RATE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `exchange-rate_list` tool
...
### Call the `exchange-rate_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `exchange-rate_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `exchange-rate_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EXCHANGE_RATE_APIKEY` | API key sent with every request. |
| `EXCHANGE_RATE_BASE` | Optional override of the API base URL. |
...
```sh
./exchange-rate-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"exchange-rate_load","arguments":{"entity":"latest","query":{"id":1}}}}'
```

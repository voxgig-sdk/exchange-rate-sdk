...
(0, node_test_1.describe)('LatestDirect', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when EXCHANGE_RATE_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('EXCHANGE_RATE_TEST_LIVE'));
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.ExchangeRateSDK({
    ...
    (0, node_test_1.test)('direct-load-latest', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-latest', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.EXCHANGE_RATE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.ExchangeRateSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['EXCHANGE_RATE_TEST_LATEST_ENTID'];
    ...
    const client = new __1.ExchangeRateSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=LatestDirect.test.js.map
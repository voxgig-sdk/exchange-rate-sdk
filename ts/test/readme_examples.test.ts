...
//     (\`new ExchangeRateSDK(...)\` / \`ExchangeRateSDK.test(...)\` become
//     \`ExchangeRateSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ExchangeRateSDK'
...
const TEST_SEED = {"entity":{"latest":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ExchangeRateSDK(...)\` and any
// \`ExchangeRateSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ExchangeRateSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ExchangeRateSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

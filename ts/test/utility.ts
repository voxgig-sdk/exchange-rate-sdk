...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.EXCHANGERATE_TEST_LIVE ||
    'TRUE' === process.env.EXCHANGERATE_TEST_OVERRIDE
  ) {
  ...
  m.EXCHANGERATE_TEST_EXPLAIN = process.env.EXCHANGERATE_TEST_EXPLAIN || m.EXCHANGERATE_TEST_EXPLAIN

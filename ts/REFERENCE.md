# ExchangeRate TypeScript SDK Reference

Complete API reference for the ExchangeRate TypeScript SDK.
...
```ts
new ExchangeRateSDK(options?: object)
```
...
#### `ExchangeRateSDK.test(testopts?, sdkopts?)`
...
**Returns:** `ExchangeRateSDK` instance in test mode.
...
Alias for `ExchangeRateSDK.test()`.
...
Return the parent `ExchangeRateSDK` instance.

...
class ExchangeRateMakeRequest
{
    public static function call(ExchangeRateContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new ExchangeRateResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new ExchangeRateResponse($fetched);
        } else {

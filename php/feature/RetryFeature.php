...
// ExchangeRate SDK retry feature
...
class ExchangeRateRetryFeature extends ExchangeRateBaseFeature
{
    ...
    public function init(ExchangeRateContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ExchangeRateContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ExchangeRateContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ExchangeRateContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

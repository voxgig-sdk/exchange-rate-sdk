...
// ExchangeRate SDK Latest entity
...
class LatestEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new LatestEntity($this->_client, $opts);
    }

    /**
     * @param Latest|array $args Latest data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ExchangeRateHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Latest|array The current Latest data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Latest fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ExchangeRateHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Latest fields).
     */
     ...
     *
     * @param LatestLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed LatestLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Latest|array The loaded Latest as an assoc-array at the
     *   SDK boundary; throws ExchangeRateError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ExchangeRateHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }

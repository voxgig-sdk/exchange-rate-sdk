# ExchangeRate SDK utility: make_request
...
module ExchangeRateUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ExchangeRateResponse.new({})
    ...
    if fetch_err
      ...
      response = ExchangeRateResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ExchangeRateResponse.new(fetched)
    else

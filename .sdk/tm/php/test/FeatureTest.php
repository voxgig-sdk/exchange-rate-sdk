...
class FtCtrl extends ExchangeRateControl
{
...
class FtClient
{
    ...
    private ExchangeRateUtility $utility;

    public function __construct(ExchangeRateUtility $utility, array $options)
    {
    ...
    public function get_utility(): ExchangeRateUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ExchangeRateContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ExchangeRateContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new ExchangeRateUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (ExchangeRateContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ExchangeRateConfig::make_feature($name);
    ...
    private static function buildUrl(ExchangeRateSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ExchangeRateError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ExchangeRateResponse($fetched);
        }
    ...
    private function fail(ExchangeRateContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ExchangeRateContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

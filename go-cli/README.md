...
boru-driven command-line client **and** interactive REPL for the ExchangeRate
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/exchange-rate-cli)
make build

# 2. See usage (words, entities, env vars)
./exchange-rate-cli --help

# 3. Provide credentials once, via the environment
export EXCHANGE_RATE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./exchange-rate-cli load 1 latest            # {id:1} shorthand
./exchange-rate-cli load '{id:1}' latest       # explicit match map

# 5. Override the API base URL for a single call
EXCHANGE_RATE_BASE=https://api.example.com ./exchange-rate-cli load 1 latest

# 6. No arguments -> interactive REPL
./exchange-rate-cli
exchange-rate> load 1 latest
exchange-rate> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/exchange-rate-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/exchange-rate-cli load 1 latest
   ```
...
```sh
./exchange-rate-cli load 1 latest          # scalar shorthand for {id:1}
./exchange-rate-cli load '{id:1}' latest     # explicit match map
```
...
./exchange-rate-cli load 1 latest
```
...
Run with no arguments to open a REPL (prompt `exchange-rate>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `latest`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/exchange-rate-cli`. |
...
(`./exchange-rate-cli <expr>`) and interactively (the REPL), and expressions compose the

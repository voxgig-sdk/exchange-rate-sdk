// Command ExchangeRate-cli is a CLI + REPL for the ExchangeRate SDK,
...
const prompt = "exchange-rate"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "latest"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      EXCHANGE_RATE_APIKEY (api key), EXCHANGE_RATE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: EXCHANGE_RATE_APIKEY carries the API key and
	// EXCHANGE_RATE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("EXCHANGE_RATE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("EXCHANGE_RATE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewExchangeRateSDK(opts)
